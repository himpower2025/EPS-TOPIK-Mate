      ...
      <nav className="flex items-center justify-between px-6 py-4 sticky top-0 bg-white/95 backdrop-blur-xl z-20 border-b border-gray-100 pt-[calc(env(safe-area-inset-top)+0.5rem)]">
        <div className="flex items-center gap-3">
          <div className="relative w-10 h-10">
            <div className="absolute top-0 left-0 bg-indigo-600 rounded-lg text-white text-[10px] font-black w-7 h-7 flex items-center justify-center border-2 border-white z-10 shadow-sm">Kor</div>
            <div className="absolute bottom-0 right-0 bg-purple-500 rounded-lg text-white text-[10px] font-black w-7 h-7 flex items-center justify-center border-2 border-white shadow-sm">A</div>
          </div>
      ...
      <div className="flex-1 flex flex-col px-6 pt-12 pb-safe max-w-4xl mx-auto w-full">
        <div className="text-center mb-12">
           ...
           <p className="text-gray-500 mb-10 max-w-md mx-auto text-base md:text-xl leading-relaxed font-medium">
             Unlimited practice exams, AI listening conversations, and instant score analysis for global candidates.
           </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-20">
           {[
             { icon: <Zap className="w-5 h-5" />, title: "Infinite Exams", desc: "Unique AI questions every time." },
             { icon: <PlayCircle className="w-5 h-5" />, title: "Native Speakers", desc: "Dual voice AI for dialogue." },
             { icon: <ShieldCheck className="w-5 h-5" />, title: "Skill Analysis", desc: "Deep feedback on weak points." }
           ].map((f, i) => (
