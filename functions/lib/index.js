...
exports.generateQuestions = exports.generateImage = exports.analyzePerformance = void 0;
...
// ── 1. 성적 분석 ─────────────────────────────────────────────
exports.analyzePerformance = (0, https_1.onRequest)({ secrets: [GEMINI_API_KEY], cors: false, timeoutSeconds: 60 }, async (req, res) => {
    if (setCors(req, res))
        return;
    if (req.method !== 'POST') {
...
SESSION DATA:
- Mode: ${session.mode}
- Questions: ${JSON.stringify((session.questions || []).map((q) => ({
                category: q.category,
...
Return JSON.`,
            config: {
                ...
                responseSchema: {
                    ...
                    properties: {
                        overallAssessment: { type: genai_1.Type.STRING },
                        ...
                        studyPlan: { type: genai_1.Type.STRING },
                    },
...
// ── 2. 즉석 이미지 생성 ──────────────────────────────────────
exports.generateImage = (0, https_1.onRequest)({ secrets: [GEMINI_API_KEY], cors: false, timeoutSeconds: 60, memory: '512MiB' }, async (req, res) => {
    if (setCors(req, res))
        return;
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
...
// ── 3. AI 문제 생성 (Pro 플랜) ────────────────────────────────
exports.generateQuestions = (0, https_1.onRequest)({ secrets: [GEMINI_API_KEY], cors: false, timeoutSeconds: 120, memory: '512MiB' }, async (req, res) => {
    if (setCors(req, res))
        return;
    if (req.method !== 'POST') {
        res.status(405).send('Method Not Allowed');
    ...
    const difficultyContext = plan === 'free'
                ...
                responseSchema: {
                    ...
                    items: {
                        ...
                        properties: {
                            ...
                            type: { type: genai_1.Type.STRING, enum: ['READING', 'LISTENING'] },
